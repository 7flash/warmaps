...
- [x] ~~**VirtualFeed renderSlice TypeError**~~ — ✅ DONE. Root cause: `tsconfig.json` sets `jsxImportSource: melina/client` (VDOM), so feed `renderItem` functions returned VNodes not HTMLElements. `renderSlice()` crashed on `.style` access. Fix: use `render(vnode, wrapper)` to materialize into real DOM. Also added null guards in renderSlice and coord validation in spotlight.tsx.
