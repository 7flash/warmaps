...
/** POST /api/geo-pins — record a confirmed geo-pin + broadcast to WS subscribers */
export async function POST(req: Request) {
    ...
    // Broadcast to all connected geo-pins WebSocket clients
    try {
        const { warmapsServer } = await import('../../../server')
        warmapsServer.publish('geo-pins', JSON.stringify({ type: 'new-pin', pin }))
    } catch { /* server not available during tests */ }
