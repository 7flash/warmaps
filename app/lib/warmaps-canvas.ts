        ...
        overlay.innerHTML = `
            ...
            ">
                ...
                <div style="display: grid; grid-template-columns: 50px 1fr; gap: 8px 12px; font-size: 13px;">
                    ...
                    <kbd style="background: rgba(51,65,85,0.6); padding: 2px 8px; border-radius: 4px; text-align: center; color: #67e8f9; font-weight: 600;">⌘K</kbd>
...
function getCommands(): PaletteCommand[] {
    const cmds: PaletteCommand[] = [
        // Canvas
        { id: 'fit', label: 'Fit All Containers', category: '⬛ Canvas', action: () => fitAllContainers() },
        { id: 'arrange', label: 'Auto-Arrange Grid', category: '⬛ Canvas', action: () => autoArrangeContainers() },
        { id: 'reset-zoom', label: 'Reset Zoom to 1:1', category: '⬛ Canvas', action: () => { if (_gd) { const s = _gd.state.getSnapshot(); _gd.state.set(1, s.offsetX, s.offsetY); } } },
        { id: 'reset-all', label: 'Reset Pan + Zoom', category: '⬛ Canvas', action: () => { if (_gd) _gd.state.set(1, 0, 0); } },
        ...
        // Help
        { id: 'help', label: 'Keyboard Shortcuts', category: '❓ Help', action: () => { /* will trigger via existing function */ } },
    ];
    return cmds;
}

function flyToLocation(lat: number, lon: number, zoom: number) {
    // Use the map reference from state (imported at module level in page.client)
    try {
        const { map } = require('./state');
        if (map?.flyTo) {
            map.flyTo({ center: [lon, lat], zoom, duration: 2000 });
        }
    } catch {
        // Fallback: try window global
        const m = (window as any).__warmaps_map;
        if (m?.flyTo) m.flyTo({ center: [lon, lat], zoom, duration: 2000 });
    }
}
